...
**Foto profissional**
O hero e a seção "Sobre" usam um placeholder ilustrado em SVG (silhueta) até
você ter uma foto profissional definitiva. Para trocar:
1. Coloque o arquivo em `public/` (ex: `public/isabella.jpg`).
2. Em `components/Hero.tsx` e `components/About.tsx`, troque o bloco `<svg>...</svg>`
   por um componente `<Image src="/isabella.jpg" alt="Isabella Cortez" fill className="object-cover" />`
   (importe `Image` de `"next/image"`).
